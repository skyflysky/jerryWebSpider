...
public class TuwanAlbumSpiderController extends BaseController
{
	...
	public DeferredResult startSpider(Integer start, Integer end)
	{
		...
		ThreadFactory namedThreadFactory = new ThreadFactoryBuilder().setNameFormat("tuwanAlbumSpider-pool-%d").build();
	...
	public DeferredResult startDownLoadImageZipPackage(Integer start, Integer end)
	{
		...
		ThreadFactory namedThreadFactory = new ThreadFactoryBuilder().setNameFormat("tuwanAlbumDownloader-pool-%d")
				.build();
		ExecutorService singleThreadPool = new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS,
				new LinkedBlockingQueue<Runnable>(1024), namedThreadFactory, new ThreadPoolExecutor.AbortPolicy());
