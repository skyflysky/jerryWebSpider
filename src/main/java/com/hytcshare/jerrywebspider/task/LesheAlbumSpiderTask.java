...
public class LesheAlbumSpiderTask implements Runnable
{
	...
	public void run()
	{
		...
		OOSpider.create(new LesheAlbumPageProcessor()).addUrl(welfareUrl).addPipeline(new ConsolePipeline())
				.addPipeline(new LesheAlbumMysqlPipeline(LesheAlbumImagesService)).thread(5).run();
