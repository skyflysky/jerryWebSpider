...
public class LesheSpiderTask implements Runnable
{
	...
	public void run()
	{
		...
		{
			throw new SpiderException(ExceptionEnum.COLLECTION_NAME_ILLEGAL.getCode(),
					ExceptionEnum.COLLECTION_NAME_ILLEGAL.getDes());
		}
		...
		for (String album : albums)
		{
			...
			OOSpider.create(new LeshePageProcessor())
					...
					.addPipeline(new ConsolePipeline()).addPipeline(new LesheMysqlPipeline(lesheImagesService))
					.thread(5).run();
		}
